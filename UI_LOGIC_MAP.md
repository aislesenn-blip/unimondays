# Playbook Ecosystem - Full UI Logic & Backend Wiring Map

**Role:** System Engineer / Fullstack Integrator
**Objective:** Comprehensive blueprint for connecting the frontend UI to the Supabase backend.
...
| **Footer** | `Partnerships` | Link | Scrolls to section | - | - | - | Public | Section expanded with new partners. |
| | `FAQs` | Link | Scrolls to section | - | - | - | Public | Section expanded with more questions. |
| | `Privacy Policy` | Link | Navigates to Policy | `GET /privacy` | - | - | Public | New page created. |
...
## 5. Group Management (Logic & UI)

**Component:** `src/components/dashboard/GroupManagement.tsx`

| Page / Component | UI Element | Type | Behavior | Backend Endpoint | Payload Structure | Response Mapping | Permissions | Deep Logic / Notes |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **Configuration** | `Grouping Method` | Select | Sets algorithm | - | `method: "random" | "manual" | "smart"` | - | Lecturer | Controls visible UI (Generate vs Manual). |
| | `Generate Groups` | Button | Triggers logic (Random/Smart) | `POST /api/sessions/[id]/groups/generate` | `{ size, method }` | `[ { id, name, members: [] } ]` | Lecturer | **Random**: Shuffles students.<br>**Smart**: Uses grade history. |
| **Manual Selection** | `Create New Group` | Button | Adds empty group | `POST /api/groups` | `{ sessionId, name }` | `{ id, name, members: [] }` | Lecturer | Creates a bucket for dragging/assigning. |
| | `Assign to...` | Dropdown | Moves student to group | `PATCH /api/groups/[id]/add-member` | `{ studentId }` | `{ success: true }` | Lecturer | Moves from `Unassigned` -> `Group`. |
| | `Remove Member` | Button (X) | Removes student from group | `PATCH /api/groups/[id]/remove-member` | `{ studentId }` | `{ success: true }` | Lecturer | Moves from `Group` -> `Unassigned`. |
| **Active Groups** | `3 Dots Menu` | Dropdown | Group Actions | - | - | - | Lecturer | - |
| | `View Details` | Item | Shows info | `GET /api/groups/[id]` | - | `{ id, name, members }` | Lecturer | - |
| | `Edit Name` | Item | Renames group | `PATCH /api/groups/[id]` | `{ name }` | - | Lecturer | - |
| | `Delete Group` | Item | Removes group | `DELETE /api/groups/[id]` | - | - | Lecturer | Moves members to "Unassigned". |

### Manual Group Selection Logic Flow
1.  **Initialization**: Lecturer selects "Manual Selection". System fetches all enrolled students and existing groups.
2.  **Unassigned Pool**: Students not in `GroupMembers` table appear in the "Unassigned" list.
3.  **Group Creation**: Lecturer clicks "Create Group" to spawn a new `Group` record.
4.  **Assignment**: Lecturer clicks "Assign to..." on a student or drags them (future) to a group. This triggers `add-member`.
5.  **State Update**: The student ID is linked to the Group ID in `GroupMembers`. The UI updates immediately (Optimistic).

### Student Group Awareness (Student Portal)
*   **Logic**: When a student logs in, the dashboard calls `GET /api/student/me`.
*   **Response**: Includes `groups: [{ id, name, sessionId }]`.
*   **UI**: A "My Group" card appears on their Session Dashboard showing their group name and peers.

---

## 6. Work / Assessment Details (`src/app/dashboard/sessions/[id]/work/[workId]/page.tsx`)
...
| **Individual Row** | `3 Dots Menu` | Dropdown | Student Actions | - | - | - | Lecturer | - |
| | `View Submission` | Item | Opens viewer | `GET /api/submissions/[id]` | - | `{ url, content }` | Lecturer | - |
| | `Flag for Review` | Item | Marks for review | `PATCH /api/submissions/[id]/flag` | `{ reason }` | `{ status: "FLAGGED" }` | Lecturer | `GRADED` → `FLAGGED` |
| | `Reset Grade` | Item | Clears score | `PATCH /api/submissions/[id]/reset` | - | `{ status: "SUBMITTED" }` | Lecturer | `GRADED` → `SUBMITTED`. Clears score. |
...
| **Group View** | `My Group` | Card | Shows group members | `GET /api/groups/my-group?sessionId=...` | - | `{ name, members: [] }` | Student | Visible only if assigned. |

---

## 8. Continuous Assessment (CA) Logic
