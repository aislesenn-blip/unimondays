...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // 2. Process Jobs (PARALLEL EXECUTION WITH LIMIT)
    const limit = pLimit(2);

    await Promise.all(
        jobs.map((job) =>
            limit(async () => {
                if (rateLimitHit) return; // Abort if a rate limit was hit by another worker
                ...
                try {
                    ...
                    if (isRateLimit) {
                        ...
                        rateLimitHit = true;
                        return; // STOP PROCESSING THIS JOB AND SIGNAL OTHERS TO ABORT
                    }
