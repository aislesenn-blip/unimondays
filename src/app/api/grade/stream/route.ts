...
4. ABSENCE HANDLING: If the student completely skipped a question, you MUST return an empty string ("") for that Question ID.`;
        ...
        // Dictionary kwa ajili ya usahihishaji wa haraka (O(1) lookup)
        ...
        for (let i = 0; i < parsedRubricItems.length; i += CONCURRENCY_LIMIT) {
            ...
            const batchPromises = batch.map(async (rubricItem: any) => {
                ...
                // 🚨 ABSOLUTE ISOLATION TRIGGER: Kama hakujibu, Mpe Ziro moja kwa moja.
                ...
                // PROMPT YAKO YA KI-ARCHITECT (Imebaki kama ilivyokuwa, ila inasoma Isolated Answer)
...
- Under no circumstances shall the score exceed ${trueMax}.`;
        ...
        try {
            const regNoResponse = await generateObject({
                model: google('gemini-2.5-pro'),  // TUNATUMIA PRO HAPA PIA
