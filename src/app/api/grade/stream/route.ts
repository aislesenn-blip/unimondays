...
// ============================================================================
// 🧱 1. ZOD SCHEMAS (ZERO COMPROMISE STANDARDS)
// ============================================================================
...
// Schema yako halisi ya Grading: Sijabadilisha hata koma.
...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        // Kazi: Kusoma maneno 10K na kutenganisha majibu bila kupoteza Data
        // ============================================================================
...
2. CONTEXT PRESERVATION: If a student scattered their answer for Q1 across page 1 and page 10, intelligently stitch those exact parts together into a single string for Q1.
3. ZERO HALLUCINATION & NO GRADING: Do NOT correct their spelling. Do NOT grade. Do NOT invent text. Your ONLY job is to extract verbatim what they wrote and route it.
4. ABSENCE HANDLING: If the student completely skipped a question and wrote absolutely nothing for it, you MUST return an empty string ("") for that Question ID.`;

        const { object: routingObject } = await generateObject({
            model: google('gemini-2.5-pro'), // STRICTLY PRO. NO FLASH.
        ...
        // Dictionary kwa ajili ya O(1) lookup
        ...
        for (let i = 0; i < parsedRubricItems.length; i += CONCURRENCY_LIMIT) {
            ...
            const batchPromises = batch.map(async (rubricItem: any) => {
                ...
                // 🚨 ABSOLUTE ISOLATION TRIGGER: Kama hakujibu, Mpe Ziro bila kuita API
                ...
                // 🚨 PROMPT YAKO YA KI-ARCHITECT (Imerudishwa kama ilivyokuwa, ila inasoma Isolated Answer)
...
- Under no circumstances shall the score exceed ${trueMax}.`;
            ...
            batchResults.forEach((result, index) => {
                if (result.status === 'fulfilled' && result.value) {
                    ...
                    finalBreakdown.push({ question: failedItem.questionId, thoughtProcess: "System Error.", score: 0, max: Number(failedItem.maxScore) || 0, feedback: "API Error encountered.", evidenceSnippet: "" });
                }
            });
        }
        ...
        try {
            const regNoResponse = await generateObject({
                model: google('gemini-2.5-pro'),  // PURE PRO. NO FLASH.
                system: "Extract the student's registration number from the text. Return UNKNOWN if none is found.",
        ...
        } catch (e) { console.error("[FATAL-GRADING] Failed to update submission status:", e); }
