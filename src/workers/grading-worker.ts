...
export async function handleAiGrade(job: any) {
    ...
    try {
        ...
        // 2. MAP PHASE: Synchronous Native OCR
        // If the ExtractedChunk table is empty, we process the student PDF directly within this worker.
        ...
        if (chunks.length === 0 && submission.filePath) {
            if (DEBUG_MODE) console.log(`[DEBUG] [OCR_STAGE] No existing chunks found. Performing Native OCR inside worker...`);

            // Extract text natively
            const buffer = await withRetries(() => readFile(submission.filePath!, 'exam_pdfs'));

            // Assuming extractPagesMultimodal reads the full buffer and returns an array of { page: number, text: string }
            const extractedPages = await withRetries(() => extractPagesMultimodal(buffer));

            if (extractedPages && extractedPages.length > 0) {
                 // Save the extracted pages natively to ExtractedChunk for debugging/idempotency
                 for (let i = 0; i < extractedPages.length; i++) {
                      const pageData = extractedPages[i];
                      const confidenceMatch = pageData.text.match(/\[CONFIDENCE:\s*([\d\.]+)\]/i);
                      const confidenceScore = confidenceMatch ? parseFloat(confidenceMatch[1]) : 1.0;
                 ...
                 // Update total chunks count for UI progression
                 await prisma.submission.update({ where: { id: submission.id }, data: { totalChunks: chunks.length, processedChunks: chunks.length } });
                 if (DEBUG_MODE) console.log(`[DEBUG] [OCR_STAGE] Native OCR successful. Extracted ${chunks.length} pages.`);
            } else {
                 throw new Error("OCR returned no text for this document.");
            }
        }

        // Update UI to 'Grading...' visually
