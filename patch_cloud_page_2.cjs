...
// The string literal in dropzoneHTML wasn't properly balanced in the replacement. Let's fix it safely.
// First, revert the whole file back to the git head so we can patch cleanly
const execSync = require('child_process').execSync;
execSync('git checkout src/app/dashboard/cloud-marking/page.tsx');
content = fs.readFileSync(targetPath, 'utf8');

const importTus = `import * as tus from 'tus-js-client';\n`;
if (!content.includes('tus-js-client')) {
    content = content.replace('import { createClientComponentClient } from "@supabase/auth-helpers-nextjs";', `${importTus}import { createClientComponentClient } from "@supabase/auth-helpers-nextjs";`);
}
  ...
  const handleDrop = (e: React.DragEvent, field: string) => {
      ...
      if (e.dataTransfer.files && e.dataTransfer.files[0]) {
          if (field === "cloudLink") {
             ...
             handleFileUpload({ target: { files: e.dataTransfer.files } } as any, field);
          }
      }
  };
  ...
  const handleTusUpload = async (file: File) => {
        ...
        try {
            ...
            const upload = new tus.Upload(file, {
                ...
                onSuccess: () => {
                    setCloudLink(fileName); // Store just the path for backend
...
content = content.replace('const supabase = createClientComponentClient();', replaceFileUpload);
...
// Safely replace the old input area.
// Look for the specific div containing the Label for Bulk Exams PDF.
const startIndex = content.indexOf('<Label>Bulk Exams PDF (Max 2GB)</Label>');
// Find the preceding <div className="space-y-2">
